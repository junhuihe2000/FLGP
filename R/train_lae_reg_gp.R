...
#' @param lower the lower bound on theta, defaulting value `c(1e-3,1e-3)`.
...
train_lae_reg_gp <- function(eigenpair, Y, K, approach="posterior",
                             theta0=NULL, lower = c(1e-3, 1e-3), upper = c(100, Inf)) {
  m = length(Y)
  ...
  if(approach=="marginal") {
    ...
    opt = stats::optim(theta0,
                       ...
                       lower = lower,
                       upper = upper)
    cat("For local anchor embedding, optimal theta = [",opt$par,"], log marginal likelihood is",-opt$value,".\n")
    ...
    opt = stats::optim(theta0,
                       ...
                       lower = lower,
                       upper = upper)
    cat("For local anchor embedding, optimal theta = [",opt$par,"], log posterior is",-opt$value,".\n")
  } else {
