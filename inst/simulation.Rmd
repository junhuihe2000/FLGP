...
```{r}
S = 100
res_pred = fit_rbf_regression_gp_rcpp(X,Y,X_new,S,approach = "posterior", sampling = "random")
Y_pred = res_pred$Y_pred
sum((Y_new-Y_pred$test)^2)/m_new
```
...
```{r, include=FALSE}
S = 100
res_pred_rbf = fit_rbf_regression_gp_rcpp(X.train,Yobs.train,X.test,S,noise="same")
```

```{r}
sqrt(res_pred_rbf$pars[-1])
```

```{r}
Y_pred_rbf = res_pred_rbf$Y_pred
sqrt(sum((Y.train-Y_pred_rbf$train)^2)/m/q)
sqrt(sum((Y.test-Y_pred_rbf$test)^2)/(n-m)/q)
```
