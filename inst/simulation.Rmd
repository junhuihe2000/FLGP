...
void refvec(const MatrixXd & mm) {
  const MatrixXd & mat = mm;
  cout << mat.rows() << "," << mat.cols() << endl;
  cout << (mat.array()*mat.array()).sum() << endl;
}

// [[Rcpp::export(test)]]
void test(NumericVector v) {
  Map<MatrixXd> mat = as<Map<MatrixXd>>(v);
  cout << mat << endl;
}
```
...
```{r}
sqrt(sum((Y.train-Y_pred_lkflag$train)^2)/m)
...
```{r}
sqrt(sum((Y.train-Y_pred_skflag$train)^2)/m)
